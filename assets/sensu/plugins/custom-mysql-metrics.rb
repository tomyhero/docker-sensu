...
class Mysql2Graphite < Sensu::Plugin::Metric::CLI::Graphite
  ...
  def run
	...
	mysql_host = section['host']
      	mysql_shorthostname = mysql_host.gsub('.','_')
      ...
      results.each do |row|
        metrics.each do |category, var_mapping|
          if var_mapping.key?(row['Variable_name'])
            output "#{mysql_shorthostname}.mysql.#{category}.#{var_mapping[row['Variable_name']]}", row['Value']
          end
        end
      end
      ...
      begin
        ...
        slave_results.first.each do |key, value| # rubocop:disable Style/Next
          if metrics['general'].include?(key)
            ...
            output "#{mysql_shorthostname}.mysql.general.#{metrics['general'][key]}", value
          end
        end
      rescue => e
        ...
        variables_results.each do |row|
          metrics[category].each do |metric, desc|
            if metric.casecmp(row['Variable_name']) == 0
              output "#{mysql_shorthostname}.mysql.#{category}.#{desc}", row['Value']
            end
          end
        end
      rescue => e
